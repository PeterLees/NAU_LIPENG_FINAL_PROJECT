...
+(void)removeRequestBlock{
    [NSURLProtocol unregisterClass:[LPURLProtocol class]];
}
...
+(void)handleRequest:(requestBlock)requestBlock responseBlock:(responseBlock)responseBlock sessionBlock:(sessionBlock)sessionBlock {
    LPURLProtocol *urlProtocol = [LPURLProtocol sharedInstance];
    ...
    urlProtocol.requestBlock = ^NSURLRequest *(NSURLRequest *request) {
        ...
        if(isEnableHttpDns){
            ...
            NSString *ipStr = [LPHttpIPGet getIPArrFromLocalDnsWithUrlStr:newRequest.URL.host];
...
+(void)blockRequest{
    if(![LPURLProtocol sharedInstance].requestBlock){
        [self handleRequest:^NSURLRequest *(NSURLRequest *request) {
